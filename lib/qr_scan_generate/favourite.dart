    ...
    Widget build(BuildContext context) {
      return Scaffold(
          appBar: AppBar(
              ...
              title: Text('favourite'.tr,style:TextStyle(fontSize: 40)),
              actions: [
                IconButton(onPressed: (){
                  Get.defaultDialog(
                      ...
                      actions:[TextButton(onPressed:(){Get.back();}, child:Text('Cancel'.tr,
                        ...
                        if(type!='All')
                          db.delete_all(type:type.value,favourite: true.toString());
                        else
                          db.delete_all(favourite: true.toString());
                        setState((){});}, child:Text('Confirm'.tr))]
                  ); }, icon:Icon(Icons.delete)) ,
                PopupMenuButton(itemBuilder: (context){
                  return order.map((e){
                    return PopupMenuItem(
                        child: Text(e.tr),onTap:(){
                      if(e=='name')
          ...
          body: Column(children:[
            ...
            Expanded(child: FutureBuilder(
                future: db.query(d: orders.value,type: type.value,favourite: true.toString()),
                ...
                Column(
                    children:h.map((e){
                      ...
                      try {
                        ...
                        if (a.favourite==false.toString()) return Container();
                        return ListTile(
                            onTap:(){
                          Get.to(()=>show(code:a,maps: map));} ,
                          ...
                          return order1.map((e){
                            return PopupMenuItem(
                              ...
                              if(e=='delete')
                                ...
                                db.updates(QR({'id':a.id,'content':a.content,'date':a.date,'type':a.type,'favourite':false.toString(),'creator':a.creator}));
                              setState((){});});
                          }).toList();
                        }),
                            ...
                            subtitle: Column(
                                ...
                                children: [
                                  Padding(padding:EdgeInsets.only(bottom:5) , child: Text(map.values.first,style: TextStyle(fontSize: 21,fontWeight:FontWeight.bold,fontStyle: FontStyle.normal  )
